interface RetryCiButtonProps {
  onClick: () => void;
  retrying: boolean;
  title: string;
  size?: number;
  className?: string;
}

export default function RetryCiButton({ onClick, retrying, title, size = 22, className }: RetryCiButtonProps) {
  return (
    <button
      type="button"
      className={`ws-icon-btn ws-icon-btn--retry ${className ?? ''}`.trim()}
      style={{ width: size, height: size }}
      onClick={(e) => {
        e.stopPropagation();
        onClick();
      }}
      disabled={retrying}
      title={title}
    >
      <svg
        width={size * 0.55}
        height={size * 0.55}
        viewBox="0 0 48 48"
        fill="currentColor"
        style={retrying ? { animation: 'ws-spin 0.7s linear infinite' } : undefined}
      >
        <path d="M 24 5 C 16.838262 5 11 10.838262 11 18 L 11 29.878906 L 7.5605469 26.439453 A 1.50015 1.50015 0 0 0 6.484375 25.984375 A 1.50015 1.50015 0 0 0 5.4394531 28.560547 L 11.310547 34.431641 A 1.50015 1.50015 0 0 0 13.697266 34.423828 L 19.560547 28.560547 A 1.50015 1.50015 0 1 0 17.439453 26.439453 L 14 29.878906 L 14 18 C 14 12.459738 18.459738 8 24 8 C 26.59565 8 28.93698 8.9889501 30.71875 10.609375 A 1.50015 1.50015 0 1 0 32.736328 8.390625 C 30.432098 6.2950499 27.36235 5 24 5 z M 35.476562 12.978516 A 1.50015 1.50015 0 0 0 34.300781 13.578125 L 28.439453 19.439453 A 1.50015 1.50015 0 1 0 30.560547 21.560547 L 34 18.121094 L 34 30 C 34 35.540262 29.540262 40 24 40 C 21.40435 40 19.06302 39.01105 17.28125 37.390625 A 1.50015 1.50015 0 1 0 15.263672 39.609375 C 17.567902 41.70495 20.63765 43 24 43 C 31.161738 43 37 37.161738 37 30 L 37 18.121094 L 40.439453 21.560547 A 1.50015 1.50015 0 1 0 42.560547 19.439453 L 36.689453 13.568359 A 1.50015 1.50015 0 0 0 35.476562 12.978516 z" />
      </svg>
    </button>
  );
}
